...
# IDS
    ...
    - complete (if the graph is finite)
    - low memory use
    - can be used for pathfinding, topological sorting, and cycle detection with increasing depth limits
...
- **Time Complexity**: $O(b^d)$, where $b$ is the branching factor and $d$ is the depth of the shallowest goal node. This is because UCS explores nodes based on their path cost, expanding the least-cost node first.
- **Space Complexity**: $O(b^d)$, where $b$ is the branching factor and $d$ is the depth of the shallowest goal node. This is because UCS needs to store all nodes in memory, including the priority queue and explored nodes.
