...
2. **Evaluate terminal states** (leaf nodes) using:
   ...
   * a payoff function (e.g., +1 = win, −1 = loss, 0 = draw), or
...
* If a move leads to a win → leaf score = `+1`
* If a move leads to a loss → leaf score = `−1`
* If a move leads to a draw → leaf score = `0`

The values are propagated back:

* If X (Max) finds any path with `+1`, it will choose it.
* If O (Min) finds any path with `−1` (good for O, bad for X), it will enforce it.
...
1. Explore left Min → child +3 → return +3
2. Still in left Min → explore child −1 → return −1 → Min picks **min(+3, −1) = −1**
3. Backtrack → right Min → child +2 → return +2
4. Then child 0 → return 0 → Min picks **min(+2, 0) = 0**
5. Finally, Max picks **max(−1, 0) = 0**
...
* DFS implementation → only need to store one path at a time.
...
* If the game tree is searched to terminal states → **Minimax always finds the optimal move** (guarantees best outcome with perfect play).
* If search depth is limited → optimality depends on **quality of evaluation function** at cut-off depth.
...
* Best case (perfect move ordering):
  ...
  → effectively doubles the depth you can search.
...
**A:** Same result, but alpha-beta prunes nodes → faster.
...
**A:** Cut off before endgame → rely on imperfect heuristics.
...
**A:** No, it assumes zero-sum; need variants like Maxⁿ.
...
**A:** Good moves first → earlier cutoffs → fewer nodes explored.
...
✅ Now you have **short, exam-style answers** to the key performance analysis questions.
...
* **Minimax explores all nodes** → exponential blow-up.
* **Alpha–Beta skips useless branches** → big speed-up.
* Works best when **good move ordering** is applied.

---

👉 Do you want me to **draw a step-by-step pruning trace** (with α and β updates at each node) for a small tree so you can *see* how branches get cut off?
